    ...
    async execute(client, interaction) {
        ...
        async function getUserMessageCount(userId) {
            try {
                ...
                if (userData?.staffPoint) {
                    embed.addFields({
                        name: "> Staff Points",
                        value: `**${
                            userData?.staffPoint?.toFixed(4) || "N/A"
                        }**`,
                    });
                }

                if (userData?.eventPoint) {
                    embed.addFields({
                        name: "> Event Points",
                        value: `**${
                            userData?.eventPoint?.toFixed(4) || "N/A"
                        }**`,
                    });
                }

                if (userData?.staffMessage) {
                    embed.addFields({
                        name: "> Staff Messages",
                        value: `**${userData?.staffMessage || "N/A"}**`,
                    });
                }

                if (userData?.eventMessage) {
                    embed.addFields({
                        name: "> Event Messages",
                        value: `**${userData?.eventMessage || "N/A"}**`,
                    });
                }
