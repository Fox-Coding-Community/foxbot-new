...
const rolesToRemove = [
    "1270778782822957057", "855055867775025152", 
...
async function setVacation(client, interaction) {
    ...
    if (user.id !== interaction.user.id) {
        if (!executor.roles.cache.has(client.staffRole) || !executor.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return interaction.editReply("> :x: You do not have permission to put others on vacation.");
        }

        const targetMember = await interaction.guild.members.fetch(user.id);
        if (!targetMember || !targetMember.roles.cache.has(client.staffRole)) {
            return interaction.editReply("> :x: The target user does not have the staff role and cannot be put on vacation.");
        }
    }
    ...
    if (durationDays < 2 || durationDays > 30) {
        return interaction.editReply("> :x: The vacation duration must be between 2 and 30 days!");
    }
    ...
    .setAuthor({ name: "Vacation", iconURL: `${client.user.displayAvatarURL({ dynamic: true })}` })
    ...
    .addFields(
        ...
        { name: "Admin", value: `${interaction.user || "None"}`, inline: true },
    ...
    interaction.editReply(`> ✅ ${user} has been put on vacation for \`${durationDays}\` days! Reason: \`${reason}\``);
}
        ...
        options: [
            {
                ...
                options: [
                    {
                        name: "duration",
                        description: "Specify the duration of the vacation (2 to 30 days)",
                        type: 4,
                        required: true,
                    },
                    {
  ...
  async execute(client, interaction) {
    ...
    loadVacations: async function(client) {
      ...
      const logChannel = await LogChannel.findOne({});
            const vacation_channel = logChannel ? guild.channels.cache.get(logChannel.vacation_channelId) : null;
...
const targetMember = await getMember(client, "300481238773399553", vacation.userId);
                ...
                if(vacation_channel) {
                    const embed = new EmbedBuilder()
                    .setColor("Green")
                    .setAuthor({ name: "Vacation Removed", iconURL: `${client.user.displayAvatarURL({ dynamic: true })}` })
                    .addFields(
                        { name: "Member", value: `<@${mute.userId}>`, inline: true },
                        { name: "Admin", value: `None`, inline: true },
                        { name: "Reason", value: `Vacation duration expired`, inline: true }
                    )
                    .setTimestamp()
