...
async function setVacation(client, interaction) {
    ...
    if (durationDays < 30 || durationDays > 365) {
        return interaction.editReply("> :x: The vacation duration must be between 30 and 365 days!");
    }
...
module.exports = {
                ...
                options: [
                  {
                    ...
                    {
                        name: "duration",
                        description: "Specify the duration of the vacation (30 to 356 days) for admins only",
                        type: 4,
                        required: true,
                    },
                    {
  ...
  async execute(client, interaction) {
    ...
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
            return interaction.editReply("> :x: You do not have permission manage vacations for others!");
        }

    if(user.id === interaction.user.id) {
      return interaction.editReply(`:x: You can't manage your own vacation!`)
    }
