    ...
    async execute(client, interaction) {
        ...
        if (duration < 1 || duration > 24) {
            return interaction.editReply(
                "> :x: Please provide a valid duration from 1 to 24 hours!"
            );
        }
        ...
        if (member.id === interaction.user.id) {
            ...
            return interaction.editReply(
                "> :x: You cannot mute a member with a higher role than you!"
            );
        }

        if (
            member.roles.cache.has(muterole) &&
            (await Mutes.findOne({ userId: member.user.id }))
        ) {
            return interaction.editReply(
                `> :x: ${member.user} Is already muted!`
            );
        }
        ...
        if (
            ...
            await interaction.editReply(
                `> :white_check_mark: Successfully muted ${member.user} for \`${duration}\` hours! Reason: \`${reason}\``
            );
            ...
            if (mute.endTime <= now) {
                ...
                if (member.roles.cache.has(muterole)) {
                    try {
                        ...
                        console.error(
                            `Failed to remove mute role from ${member.user.tag}:`,
                            error
                        );
                    }
                }
                ...
                try {
                    ...
                    console.error(
                        `Failed to delete mute record for ${member.user.tag}:`,
                        error
                    );
                }

                const logChannel = await LogChannel.findOne({}).catch(
                    () => null
                );
                const mute_channel = logChannel
                    ? guild.channels.cache.get(logChannel.mute_channelId)
                    : null;
                ...
                if (mute_channel) {
                    ...
                    try {
                        ...
                        console.error(
                            `Failed to send unmute log for ${member.user.tag}:`,
                            error
                        );
                    }
                }
