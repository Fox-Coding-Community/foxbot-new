    ...
    async execute(client, interaction) {
            ...
            try {
                ...
                const membersWithRole = interaction.guild.members.cache
                    ...
                    .map((member) => {
                        ...
                        const secondHighestRole = sortedRoles[1] || {
                            name: "No additional roles",
                        };

                        return {
                            user: member.user,
                            displayRole:
                                highestRole.id === staffRoleId
                                    ? secondHighestRole.name
                                    : highestRole.name,
                        };
                    })
                    .sort((a, b) => {
                        ...
                        return (
                            interaction.guild.roles.cache.find(
                                (r) => r.name === b.displayRole
                            )?.position -
                            interaction.guild.roles.cache.find(
                                (r) => r.name === a.displayRole
                            )?.position
                        );
                    });
                ...
                const membersList =
                    membersWithRole
                        .map(
                            (member) => `${member.user} (${member.displayRole})`
                        )
                ...
                await interaction.editReply(
                    "> :x: There was an error fetching the staff list. Please try again later."
                );
            }
        }
        ...
        if (subcommand === "events") {
            try {
                ...
                await interaction.editReply(
                    "> :x: There was an error fetching the events list. Please try again later."
                );
            }
        }
    },
