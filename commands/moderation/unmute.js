...
const rolesMute = ["1270780308341657660", "378659419652751363", "1169283299013820526",
...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        if (!member || !interaction.guild) {
            return interaction.editReply("> :x: Unable to find the specified guild or member!");
        }
        ...
        if (member.id === interaction.user.id) {
            ...
            return interaction.editReply("> :x: You cannot unmute a member with a higher role than you!");
        }

        if (!interaction.member.permissions.has(PermissionsBitField.Flags.MuteMembers) && !rolesMute.some(role => interaction.member.roles.cache.has(role))) { 
        ...
        await interaction.editReply(`> :white_check_mark: Successfully unmuted ${member.user}!`);
